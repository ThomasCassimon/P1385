...
TEST_CASE("dr_vector_engine Copy Constructor", "[dr_vector_engine][copy][constructor]")
{
	SECTION("dr_vector_engine<unsigned short, std::allocator<unsigned short>>")
	{
		std::math::dr_vector_engine<unsigned short, std::allocator<unsigned short>> engine1 {3, 6, 9, 12, 15};

		std::math::dr_vector_engine<unsigned short, std::allocator<unsigned short>> engine2(engine1);

		REQUIRE(engine2.elements() == engine1.elements());
		CHECK(engine1.capacity() == engine2.capacity());

		for (std::size_t i = 0; i < engine1.elements(); ++i)
		{
			CAPTURE(i);
			CHECK(engine1(i) == Approx(3u * static_cast<unsigned short>(i + 1)));
			CHECK(engine2(i) == Approx(3u * static_cast<unsigned short>(i + 1)));
		}
	}

	SECTION("dr_vector_engine<float, std::allocator<float>>")
	{
		std::math::dr_vector_engine<float, std::allocator<float>> engine1 {3.0f, 6.0f, 9.0f, 12.0f, 15.0f};
		...
		{
			CAPTURE(i);
			CHECK(engine1(i) == Approx(3.0f * (static_cast<float>(i) + 1.0f)));
			CHECK(engine2(i) == Approx(3.0f * (static_cast<float>(i) + 1.0f)));
		}
	}
}
...
TEST_CASE("dr_vector_engine Two Argument Constructor", "[dr_vector_engine][two][argument][constructor]")
{
	SECTION("dr_vector_engine<unsigned short, std::allocator<unsigned short>>")
	{
		std::math::dr_vector_engine<unsigned short, std::allocator<unsigned short>> engine(3, 6);

		REQUIRE(engine.elements() == 3);
		CHECK(engine.capacity() >= 6);
	...
	SECTION("dr_vector_engine<float, std::allocator<float>>")
	{
		std::math::dr_vector_engine<float, std::allocator<float>> engine(3, 6);
